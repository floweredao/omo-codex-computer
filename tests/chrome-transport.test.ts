...
function success(result: unknown): { content: unknown[] } {
  return { content: [{ type: "text", text: JSON.stringify({ protocol: "omo-codex-computer/chrome-v1", ok: true, result }) }] };
}

function failure(error: string, phase: string): { content: unknown[] } {
  return { content: [{ type: "text", text: JSON.stringify({ protocol: "omo-codex-computer/chrome-v1", ok: false, error, phase }) }] };
}
...
describe("ChromeTransport", () => {
  it("discovers once and sends strict fixed-program requests with canonical metadata", async () => {
    ...
    expect(program).toContain("omo-codex-computer/chrome-v1");
    expect(program).toContain("setupBrowserRuntime");
    expect(program).toContain('browsers.get("chrome")');
    expect(program).toContain('browser.nameSession("OMO Chrome")');
  ...
  it("requires exactly one envelope and discards raw metadata and errors", async () => {
    ...
    const malformedResponses: unknown[] = [
      ...
      { content: [
        { type: "text", text: JSON.stringify({ protocol: "omo-codex-computer/chrome-v1", ok: true, result: { kind: "opened" } }) },
        { type: "text", text: JSON.stringify({ protocol: "omo-codex-computer/chrome-v1", ok: true, result: { kind: "opened" } }) },
      ] },
    ];
    ...
    client.responses.push({
      content: [{ type: "text", text: JSON.stringify({ protocol: "omo-codex-computer/chrome-v1", ok: true, result: { kind: "opened" } }) }],
  ...
  it("rejects error envelopes without a known program phase", async () => {
    ...
    client.responses.push({ content: [{ type: "text", text: JSON.stringify({
      protocol: "omo-codex-computer/chrome-v1", ok: false, error: "operation_failed",
    }) }] });
