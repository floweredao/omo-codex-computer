...
export function createComputerUseParameterSchemas(): Record<ComputerUseToolName, TSchema> {
  ...
  return {
    ...
    computer_use_scroll: object({
      app,
      element_index: Type.Optional(elementIndex),
      x: Type.Optional(Type.Number({ description: "The x coordinate to scroll at, when not scrolling an element." })),
      y: Type.Optional(Type.Number({ description: "The y coordinate to scroll at, when not scrolling an element." })),
