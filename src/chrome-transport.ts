...
const CHROME_PROTOCOL = "omo-codex-computer/chrome-v1" as const;
...
function buildChromeProgram(clientPath: string, payloadBase64: string): string {
  ...
  try {
    ...
    const registryKey = Symbol.for("omo-codex-computer.chrome-v1.registry");
    ...
    if (session === undefined) {
      ...
      try {
        ...
        await browser.nameSession("OMO Chrome");
      } catch {
        fail("unavailable");
      }
